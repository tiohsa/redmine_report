...
import { generateWeeklyReport, prepareWeeklyPrompt } from '../../services/scheduleReportApi';
import { addReportDetailAiComment } from '../../services/reportDetailApi';
import type { WeeklyGenerateResponse, WeeklyPrepareResponse } from '../../types/weeklyReport';
import { t } from '../../i18n';
...
export const VersionAiDialog = ({
  ...
  const destinationReady = useMemo(
    () =>
      Number.isInteger(destinationIssueId) &&
      destinationIssueId !== null &&
      destinationIssueId > 0 &&
      destinationIssueStatus === 'VALID',
    [destinationIssueId, destinationIssueStatus]
  );

  const saveEnabled = useMemo(() => {
    return !!editableMarkdown.trim() && destinationReady;
  }, [editableMarkdown, destinationReady]);
  ...
  const saveReport = async () => {
    ...
    if (!saveEnabled || !destinationIssueId) {
      setError(t('weeklyDialog.relatedIssueRequired'));
      return;
    }
    ...
    setMessage(null);
    try {
      const saveResult = await addReportDetailAiComment(projectIdentifier, {
      ...
      if (saveResult.revision) {
        setMessage(t('weeklyDialog.aiCommentSaved', { revision: saveResult.revision }));
      } else {
        setMessage(t('weeklyDialog.aiCommentSaved', { revision: '-' }));
      }
      onSaved?.();
    } catch (e) {
      setError((e as Error).message);
  ...
  return (
    <div className={reportStyles.dialogOverlay} onClick={onClose}>
      <div
        className={`${reportStyles.dialogPanel} ${reportStyles.dialogPanelLg} version-ai-dialog relative max-h-[95vh] animate-in fade-in zoom-in duration-300`}
      ...
      >
        <div className={reportStyles.dialogHeader}>
          ...
          <Button variant="icon-muted" onClick={onClose} aria-label={t('common.close')} style={{ borderRadius: '16px' }}>
            <Icon name="close" />
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-8 p-8 scrollbar-thin scrollbar-thumb-slate-200">
          <section className="report-panel space-y-4 p-6">
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-[13px] font-medium text-slate-700">
              {destinationReady ? (
                <div className="space-y-1">
                  <div>{t('weeklyDialog.relatedDetailReportIssue', { id: destinationIssueId })}</div>
                  <div className="text-[12px] text-slate-500">{t('weeklyDialog.aiCommentWillBeAdded')}</div>
                </div>
              ) : (
                <div className="text-amber-700">{t('weeklyDialog.relatedIssueRequired')}</div>
              )}
            </div>
          </section>
          ...
          <section className="report-panel space-y-4 p-6">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={preparePrompt} disabled={loadingPrepare} className="version-ai-dialog-button group relative">
            ...
            {promptText && (
              <div className="relative group animate-in zoom-in-95 duration-300">
                ...
                />
                <button
                  className="report-icon-button-muted version-ai-dialog-icon-button absolute right-4 top-4 opacity-0 shadow-sm group-hover:opacity-100"
                  title={t('common.copy')}
                  onClick={() => {
                ...
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      ...
                      strokeWidth="2"
                      d="M8 7v8a2 2 0 002 2h6a2 2 0 002-2V7a2 2 0 00-2-2h-6a2 2 0 00-2 2zM8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-2"
                    ></path>
                  </svg>
                </button>
              </div>
          ...
          <section className="report-panel space-y-4 p-6">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={submitToLlm} disabled={loadingGenerate || !promptText} className="version-ai-dialog-button group relative">
                <div className="relative z-10 flex items-center gap-2">
                  {loadingGenerate ? (
                    <svg className="animate-spin h-3.5 w-3.5" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          ...
          {error && (
            <div className="report-alert-error flex items-center gap-3 animate-in shake duration-500">
              <svg className="w-5 h-5 text-rose-500 flex-none" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
        ...
        <div className="flex flex-none items-center justify-between border-t border-gray-100 bg-white px-8 py-6">
          ...
          <div className="flex items-center gap-4">
            <Button variant="secondary" onClick={onClose} className="version-ai-dialog-button">
              {t('common.close')}
            </Button>
            <Button onClick={saveReport} disabled={!saveEnabled || loadingSave} className="version-ai-dialog-button">
              <div className="relative z-10 flex items-center gap-2">
                {loadingSave ? (
                  ...
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V3"></path>
                  </svg>
                )}
                {loadingSave ? t('weeklyDialog.saving') : t('weeklyDialog.addAiComment')}
              </div>
            </Button>
          </div>
        </div>
