  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="w-5 mr-0.5 flex-shrink-0 flex items-center justify-center">
            {node.children.length > 0 ? (
              <button
                type="button"
                className={`p-0.5 !border-0 ring-0 shadow-none bg-transparent appearance-none rounded-sm text-slate-400 hover:text-slate-700 hover:bg-slate-100/80 focus:outline-none cursor-pointer flex-shrink-0 z-10 ${DENSITY_CONFIG[density].controlButtonSize}`}
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <div className="flex items-center min-w-0 z-10 flex-1">
            {isEditing('subject') ? (
              ...
              <span
                ...
                className={`${DENSITY_CONFIG[density].subjectSize} leading-[1.1] ${depth === 0 ? 'font-semibold text-slate-800' : 'font-medium text-slate-700'} truncate hover:text-blue-700 block cursor-pointer`}
                onClick={(e) => {
                  e.stopPropagation();
                  onSelectIssue(node);
                  onViewIssue(node);
            ...
            {!isEditing('subject') ? (
              <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 ml-0.5 flex-shrink-0">
                <button
                  type="button"
                  className={`report-icon-button-muted !rounded-[6px] opacity-70 hover:opacity-100 ${DENSITY_CONFIG[density].controlButtonSize}`}
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    onAddSubIssue(node);
        ...
        <div
          className={`shrink-0 flex items-center justify-start px-1.5 border-r border-slate-200/80 self-stretch overflow-hidden ${fieldCellClass('tracker_id')}`}
        ...
        >
          {isEditing('tracker_id') && issueEditOptions ? (
            <select
              ...
              className={`w-full ${DENSITY_CONFIG[density].badgeSize} ${DENSITY_CONFIG[density].controlSelectHeight} ${DENSITY_CONFIG[density].controlPaddingX} border border-blue-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-100 bg-white text-slate-700 shadow-sm`}
              value={editingCell!.value}
              onChange={(e) => {
            ...
            <span data-testid={`tracker-display-${node.issue_id}`} className={`inline-flex max-w-full items-center justify-center rounded-[9999px] ${DENSITY_CONFIG[density].badgePaddingX} ${DENSITY_CONFIG[density].badgePaddingY} ${DENSITY_CONFIG[density].badgeSize} font-semibold font-sans truncate transition-all duration-300 ${trackerBadgeClass} group/cell:hover:ring-1 group/cell:hover:ring-blue-300`} title={node.tracker_name || ''}>
              {node.tracker_name || '-'}
            </span>
          )}
        </div>
        ...
        <div
          className={`shrink-0 flex items-center justify-start px-1.5 border-r border-slate-200/80 self-stretch overflow-hidden ${fieldCellClass('priority_id')}`}
        ...
        >
          {isEditing('priority_id') && issueEditOptions ? (
            ...
            <span data-testid={`priority-display-${node.issue_id}`} className={`inline-flex max-w-full items-center justify-center rounded-[9999px] ${DENSITY_CONFIG[density].badgePaddingX} ${DENSITY_CONFIG[density].badgePaddingY} ${DENSITY_CONFIG[density].badgeSize} font-semibold font-sans truncate transition-all duration-300 ${priorityBadgeClass}`} title={node.priority_name || ''}>
              {node.priority_name || '-'}
            </span>
          )}
        </div>
        ...
        <div
          className={`shrink-0 flex items-center justify-start px-1.5 border-r border-slate-200/80 self-stretch overflow-hidden ${fieldCellClass('status_id')}`}
        ...
        >
          {isEditing('status_id') && issueEditOptions ? (
            ...
            <span data-testid={`status-display-${node.issue_id}`} className={`inline-flex items-center justify-center ${DENSITY_CONFIG[density].statusMinWidth} ${DENSITY_CONFIG[density].badgeSize} font-bold ${DENSITY_CONFIG[density].badgePaddingX} ${DENSITY_CONFIG[density].badgePaddingY} rounded-full ${statusBg} ${statusText} shadow-sm`}>
              {statusLabel}
            </span>
          )}
        </div>

        <div className={`shrink-0 flex items-center gap-1.5 justify-start px-1.5 border-r border-slate-200/80 self-stretch overflow-hidden ${cellClass}`} style={{ width: `${columnWidths.progress}px`, minWidth: `${columnWidths.progress}px` }} onDoubleClick={(e) => startEdit('done_ratio', String(progressRatio), e)}>
          {isEditing('done_ratio') ? (
            <input
              ...
              max={100}
              step={10}
              className={`${DENSITY_CONFIG[density].progressInputWidth} ${DENSITY_CONFIG[density].progressTextSize} ${DENSITY_CONFIG[density].progressInputHeight} ${DENSITY_CONFIG[density].controlPaddingX} border border-[var(--color-brand-6)] rounded-[9999px] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary-200)] bg-white text-slate-700 shadow-sm font-sans font-medium`}
        ...
        <div
          className={`shrink-0 flex items-center justify-start gap-1 px-1.5 overflow-hidden ${fieldCellClass('assigned_to_id')}`}
        ...
        >
          {isEditing('assigned_to_id') && issueEditOptions ? (
            <select
              ...
              className={`w-full ${DENSITY_CONFIG[density].dateSize} ${DENSITY_CONFIG[density].controlSelectHeight} ${DENSITY_CONFIG[density].controlPaddingX} border border-blue-400 rounded-md focus:outline-none bg-white text-slate-700`}
              value={editingCell!.value}
              onChange={(e) => {
            ...
            <>
              <div className={`${DENSITY_CONFIG[density].avatarSize} rounded-full bg-slate-100 ring-1 ring-slate-200 flex items-center justify-center flex-shrink-0`}>
...
export function IssueTreeTable({
  ...
  return (
    <>
      <div className={`flex items-center py-2 ${DENSITY_CONFIG[density].cellPadding} bg-gray-50/80 z-20 border-b border-gray-100/50 text-slate-600 flex-shrink-0 ${DENSITY_CONFIG[density].headerHeight} box-border sticky top-0 tracking-wide font-semibold ${DENSITY_CONFIG[density].badgeSize}`}>
